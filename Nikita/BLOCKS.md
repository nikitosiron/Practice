...
contactForm: {
  ...
  directions: [   // варианты для <select name="direction">
    "Backend", "Frontend", "Тестирование", ...
  ]
}
```

- Массив `directions` — **строки**, а не объекты. Не путать с `directions[]` из Блока 5 (это про раздел «Направления работы»).
- Умышленное дублирование значений с блоком 5: чтобы админ мог менять список опций формы независимо от карточек направлений. Синхронизировать вручную (или Егор при рендере может использовать `directions[].map(d => d.title)` и игнорировать `contactForm.directions` — на усмотрение фронта, договориться при интеграции).
